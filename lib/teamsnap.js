...
modifySDK = function(sdk) {
  ...
  return wrapMethod(sdk, 'bulkDeleteMessages', function(bulkDeleteMessages) {
    return function(messages, callback) {
      var contactIds, i, len, memberIds, message, messageTypes, toRemove;
      contactIds = [];
      memberIds = [];
      messageTypes = [];
      ...
      if (toRemove != null) {
        ...
        for (i = 0, len = toRemove.length; i < len; i++) {
          message = toRemove[i];
          if ((message.contactId != null) && contactIds.indexOf(message.contactId) < 0) {
            contactIds.push(message.contactId);
          }
          if ((message.memberId != null) && memberIds.indexOf(message.memberId) < 0) {
      ...
      return bulkDeleteMessages.call(this, messages).then(function(result) {
        var params;
        if (contactIds.length || memberIds.length) {
          params = {};
          if (contactIds.length) {
            params.contactId = contactIds.join();
          }
          if (memberIds.length) {
            params.memberId = memberIds.join();
          }
          if (messageTypes.length) {
            params.messageType = messageTypes.join();
          }
...
TeamSnap = (function() {
  TeamSnap.prototype.version = '1.19.1';
