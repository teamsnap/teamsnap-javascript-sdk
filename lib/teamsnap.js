...
exports.saveBroadcastSms = function(broadcastSms, callback) {
  ...
  if (!((_ref = broadcastSms.body) != null ? _ref.trim() : void 0)) {
...
require.register("collections/divisionCustomData", function(exports, require, module) {
exports.loadDivisionCustomData = function(params, callback) {
  if (this.isId(params)) {
    params = {
      teamId: params
    };
  } else if (!(params && typeof params === 'object')) {
    throw new TSArgsError('teamsnap.loadDivisionCustomData', 'must provide a teamId or query parameters');
  }
  return this.loadItems('divisionCustomDatum', params, callback);
};

exports.saveDivisionCustomDatum = function(divisionCustomDatum, callback) {
  if (!divisionCustomDatum) {
    throw new TSArgsError('teamsnap.saveDivisionCustomDatum', '`divisionCustomDatum` must be provided');
  }
  if (!this.isItem(divisionCustomDatum, 'divisionCustomDatum')) {
    throw new TSArgsError('teamsnap.saveDivisionCustomDatum', "`divisionCustomDatum.type` must be 'divisionCustomDatum'");
  }
  return this.saveItem(divisionCustomDatum, callback);
};

});

require.register("collections/divisionCustomFields", function(exports, require, module) {
exports.loadDivisionCustomFields = function(params, callback) {
  if (this.isId(params)) {
    params = {
      teamId: params
    };
  } else if (!(params && typeof params === 'object')) {
    throw new TSArgsError('teamsnap.loadDivisionCustomFields', 'must provide a teamId or query parameters');
  }
  return this.loadItems('divisionCustomField', params, callback);
};

exports.createDivisionCustomField = function(data) {
  return this.createItem(data, {
    type: 'divisionCustomField'
  });
};

exports.saveDivisionCustomField = function(divisionCustomField, callback) {
  if (!divisionCustomField) {
    throw new TSArgsError('teamsnap.saveDivisionCustomField', '`divisionCustomField` must be provided');
  }
  if (!this.isItem(divisionCustomField, 'divisionCustomField')) {
    throw new TSArgsError('teamsnap.saveCustomField', "`divisionCustomField.type` must be 'divisionCustomField'");
  }
  if (!divisionCustomField.teamId) {
    return this.reject('You must choose a team.', 'teamId', callback);
  }
  if (!divisionCustomField.name) {
    return this.reject('You must enter a name.', 'name', callback);
  }
  if (!divisionCustomField.kind) {
    return this.reject('You must choose a type.', 'kind', callback);
  }
  return this.saveItem(divisionCustomField, callback);
};

exports.deleteDivisionCustomField = function(divisionCustomField, callback) {
  if (!divisionCustomField) {
    throw new TSArgsError('teamsnap.deleteDivisionCustomField', '`divisionCustomField` must be provided');
  }
  return this.deleteItem(divisionCustomField, callback);
};
...
exports.saveForumPost = function(forumPost, callback) {
  ...
  if (!((_ref = forumPost.message) != null ? _ref.trim() : void 0)) {
...
exports.saveForumTopic = function(forumTopic, callback) {
  ...
  if (!((_ref = forumTopic.title) != null ? _ref.trim() : void 0)) {
...
add(require('./collections/divisionCustomData'));

add(require('./collections/divisionCustomFields'));
...
types = ['user', 'assignment', 'availability', 'broadcastSms', 'contact', 'contactEmailAddress', 'contactPhoneNumber', 'customDatum', 'customField', 'divisionCustomDatum', 'divisionCustomField', 'divisionMember', 'event', 'forumPost', 'forumSubscription', 'forumTopic', 'location', 'member', 'memberEmailAddress', 'memberFile', 'memberLink', 'memberPayment', 'memberPhoneNumber', 'memberPreferences', 'opponent', 'opponentResults', 'plan', 'smsGateway', 'sponsor', 'sport', 'team', 'teamFee', 'teamFile', 'teamPreferences', 'teamPublicSite', 'teamResults', 'timeZone', 'trackedItem', 'trackedItemStatus'];
