...
var authRequest, browserStorageName, browserStore, collectionJSONMime, createAuthDialog, jsonMime, multipartMime, promises, request, sdk, sdkRequest, teamsnap;
...
module.exports = function(clientId, secret) {
  ...
  return {
    auth: function(token, cachedCollections, callback) {
      ...
      if (!token && teamsnap.apiUrl.indexOf(':3000') !== -1) {
      ...
      return sdk(authedRequest, cachedCollections, this.version, callback);
    },
...
exports.deleteSponsor = function(sponsor, callback) {
  ...
  if (sponsor.links.logo.href.length) {
    ...
    this.collections.sponsors.exec('removeSponsorLogo', params);
  }
...
exports.deleteTeamLogo = function(teamPreferencesId, callback) {
  ...
  if (!this.isId(teamsPreferencesId)) {
    throw new TSArgsError('teamsnap.deleteTeamLogo', "`teamPreferencesId` must be a valid id");
  }
  params = {
    teamPreferencesId: teamPreferencesId
  };
  return this.collections.teamPreferences.exec('removeTeamLogo', params).callback(callback);
};
...
var Item, MetaList, ScopedCollection, camelize, copy, linking, lookup, modifyModel, modifySDK, promises, revertModel, revertSDK, revertWrapMethod, types, wrapMethod, wrapSaveForNew, _ref;
...
modifySDK = function(sdk) {
  ...
  wrapMethod(sdk, 'deleteEvent', function(deleteEvent) {
    return function(event, include, callback) {
      ...
      linking.unlinkItems(toRemove, lookup);
      return deleteEvent.call(this, event, include, callback).fail(function(err) {
        linking.linkItems(toRemove, lookup);
        return err;
      }).callback(callback);
    };
...
module.exports = function(request, cachedCollections, version, callback) {
  if (typeof cachedCollections === 'function') {
    callback = cachedCollections;
    cachedCollections = null;
  }
  if ((cachedCollections != null ? cachedCollections.version : void 0) !== version) {
    cachedCollections = null;
  }
...
createSDKObject = function(request, collections, version) {
  var add, linking, scopedCollections, sdk;
  scopedCollections = {};
  Object.keys(collections).forEach(function(name) {
    var collection;
    collection = collections[name];
    return scopedCollections[name] = new ScopedCollection(request, collection);
  });
  sdk = {
    version: version,
    when: promises.when,
...
types = ['user', 'assignment', 'availability', 'contact', 'contactEmailAddress', 'contactPhoneNumber', 'customDatum', 'customField', 'event', 'location', 'member', 'memberEmailAddress', 'memberFile', 'memberLink', 'memberPhoneNumber', 'memberPreferences', 'opponent', 'plan', 'smsGateway', 'sponsor', 'sport', 'team', 'teamPreferences', 'teamPublicSite', 'teamResults', 'timeZone', 'trackedItem', 'trackedItemStatus'];
