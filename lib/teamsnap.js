...
exports.bulkDeleteMessages = function(params, callback) {
  if (Array.isArray(params)) {
    params = {
      id: params
    };
  } else if (typeof params === 'object' && this.isItem(params, 'message')) {
    params = {
      id: params.id
    };
  } else if (this.isId(params)) {
  ...
  return this.collections.messages.exec('bulkDelete', params).callback(callback);
};
...
modifySDK = function(sdk) {
  ...
  return wrapMethod(sdk, 'bulkDeleteMessages', function(bulkDeleteMessages) {
    return function(messages, callback) {
      var i, len, message, messageIds, toRemove;
      if (Array.isArray(messages)) {
        toRemove = messages;
      } else if (typeof messages === 'object' && this.isItem(messages, 'message')) {
        toRemove = [messages];
      } else {
        throw new TSArgsError('teamsnap.bulkDeleteMessages', 'Must provide an `array` of message `ids` or a `message` object to call `bulkDeleteMessages` using the persistene layer');
      }
      messageIds = [];
      for (i = 0, len = toRemove.length; i < len; i++) {
        message = toRemove[i];
        if (!this.isItem(message, 'message')) {
          throw new TSArgsError('teamsnap.bulkDeleteMessages', "Items in messages array must have a 'message' `type`");
        }
        messageIds.push(message.id);
      }
      toRemove = messages;
      linking.unlinkItems(toRemove, lookup);
      return bulkDeleteMessages.call(this, messageIds).then(function(result) {
        return result;
      }).fail(function(err) {
        linking.linkItems(toRemove, lookup);
        return err;
...
TeamSnap = (function() {
<<<<<<< 6dacaa77a9903d1b33efe02e7a40ee7e41709311
  TeamSnap.prototype.version = '1.17.0';
=======
  TeamSnap.prototype.version = '1.16.2';
>>>>>>> Added bulkDeleteMessages command to messages
