...
exports.createPartnerUserPreference = function(data) {
  return this.createItem(data, {
    type: 'partnerUserPreference'
  });
};

exports.savePartnerUserPreference = function(partnerUserPreference, callback) {
  if (!partnerUserPreference) {
    throw new TSArgsError('teamsnap.savePartnerUserPreference', "`partnerUserPreference` must be provided");
  }
  if (!this.isItem(partnerUserPreference, 'partnerUserPreference')) {
    throw new TSArgsError('teamsnap.savePartnerUserPreference', "`type` must be 'partnerUserPreference'");
  }
  if (!partnerUserPreference.userId) {
    return this.reject('You must supply a user.', 'userId', callback);
  }
  if (!partnerUserPreference.partnerPreferenceId) {
    return this.reject('You must supply a partnerPreference.', 'partnerPreferenceId', callback);
  }
  if (!partnerUserPreference.optionValue.trim()) {
    return this.reject('You must supply an optionValue.', 'optionValue', callback);
  }
  return this.saveItem(partnerUserPreference, callback);
};
...
TeamSnap = (function() {
  TeamSnap.prototype.version = '1.56.0';
