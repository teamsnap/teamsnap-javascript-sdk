...
exports.assignmentSort = function(reverse) {
...
var groupUndos, linkItem, linkItemWith, types, undoableAdd, undoableAddTo, undoableRemove, undoableRemoveFrom, unlinkItem, unlinkItemFrom, unlinkItemsFrom;
...
exports.linkItems = function(items, lookup) {
  ...
  if (Array.isArray(items)) {
    ...
    undos = items.map(function(item) {
      return linkItem(item, lookup);
    });
    return groupUndos(undos);
  } else {
    if (items.href) {
...
linkItem = function(item, lookup) {
  ...
  item.links.each(function(rel, href) {
    var related;
    if (types.isPluralType(rel)) {
      return undos.push(undoableAdd(item, rel, []));
    } else {
      if (!(related = lookup[href])) {
        return;
      }
      undos.push(undoableAdd(item, rel, related));
      return undos.push(linkItemWith(item, related));
    }
  });
  return groupUndos(undos);
};
...
linkItemWith = function(item, other) {
  ...
  if (plural && other.links.has(plural)) {
    ...
    other.links.each(function(rel, href) {
      if (href === item.href) {
        return undos.push(undoableAdd(other, rel, item));
      }
    });
  }
  return groupUndos(undos);
  ...
  item.links.each(function(rel, href) {
    ...
    if (types.isPluralType(rel)) {
      if (item[rel]) {
        ...
        return undos.push(undoableRemove(item, rel));
      }
    } else {
      related = item[rel];
      undos.push(undoableRemove(item, rel));
      return undos.push(unlinkItemFrom(item, related));
    }
  });
  return groupUndos(undos);
};
...
unlinkItemFrom = function(item, other) {
  ...
  undos = [];
  plural = types.getPluralType(item.type);
  if (plural && other.links.has(plural) && other[plural] === item) {
    undos.push(undoableRemoveFrom(other, plural, item));
  } else {
    other.links.each(function(rel, href) {
      if (href === item.href && other[rel] === item) {
        return undos.push(undoableRemove(other, rel));
      }
    });
  }
  return groupUndos(undos);
...
groupUndos = function(undos) {
  return function() {
    return undos.forEach(function(undo) {
      return undo();
    });
  };
};

undoableAdd = function(item, property, value) {
  var exists, oldValue;
  ...
  return function() {
    if (exists) {
      return item[property] = oldValue;
    } else {
      return delete item[property];
    }
  };
};

undoableAddTo = function(item, property, value) {
  if (!item[property]) {
    item[property] = [];
  }
  ...
  return function() {
    ...
    if (item[property]) {
      ...
      if (index !== -1) {
        return item[property].splice(index, 1);
      }
    }
  };
};

undoableRemove = function(item, property) {
  var oldValue;
  oldValue = item[property];
...
undoableRemoveFrom = function(item, property, value) {
  var index;
  index = item[property].indexOf(value);
  if (index === -1) {
    return (function() {});
  ...
  return function() {
    ...
    if (item[property].length === length) {
      return item[property].push(value);
    }
  };
};
...
;require.register("persistance", function(exports, require, module) {
...
exports.enablePersistance = function() {
  var sdk;
  if (lookup) {
    return;
  }
...
exports.disablePersistance = function() {
  if (!lookup) {
    return;
  }
...
modifyModel = function() {
  wrapMethod(ScopedCollection.prototype, 'save', function(save) {
    return function(item, callback) {
      return save.call(this, item).then(saveState).callback(callback);
    };
  });
  ...
  Item.prototype.saveState = function() {
    ...
    this._state._undos = [
      (function(_this) {
        return function() {
          return copy(_this._state, _this);
        };
      })(this)
    ];
    this._state.undo = linking.groupUndos(this._state._undos);
    return this;
  };
  Item.prototype.rollback = function() {
    var _ref1;
    if ((_ref1 = this._state) != null) {
      _ref1.undo();
    }
    this._state._undos.length = 1;
    return this;
  };
  return Item.prototype.link = function(rel, item) {
    ...
    if (item.id) {
      undos.push(linking.undoableAdd(this, rel + 'Id', item.id));
    }
  ...
  return wrapMethod(sdk, saveMethodName, function(save) {
    return function(item, callback) {
      ...
      if (item.id) {
        ...
        savedItem = null;
        return save.call(item).then(function(item) {
...
types = ['user', 'assignment', 'availability', 'contact', 'contactEmailAddress', 'contactPhoneNumber', 'event', 'location', 'member', 'memberEmailAddress', 'memberLink', 'memberPhoneNumber', 'memberPreferences', 'opponent', 'plan', 'sport', 'team', 'teamPreferences', 'teamPublicSite', 'trackedItem', 'trackedItemStatus'];
