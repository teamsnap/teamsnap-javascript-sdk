...
exports.bulkDeleteMessages = function(params, callback) {
  var i, id, len, msg, requestParams;
  if (!Array.isArray(params)) {
    throw new TSArgsError('teamsnap.bulkDeleteMessages', 'an array of `messages` must be provided');
  }
  requestParams = {
    id: []
  };
  for (i = 0, len = params.length; i < len; i++) {
    msg = params[i];
    if (!this.isItem(msg, 'message')) {
      throw new TSArgsError('teamsnap.bulkDeleteMessages', "Items in params array must have a 'message' `type`");
    }
    id = msg.id;
    requestParams.id.push(id);
  }
  return this.collections.messages.exec('bulkDelete', requestParams).callback(callback);
};
...
modifySDK = function(sdk) {
  ...
  return wrapMethod(sdk, 'bulkDeleteMessages', function(bulkDeleteMessages) {
    return function(params, callback) {
      ...
      toRemove = params;
      linking.unlinkItems(toRemove, lookup);
      return bulkDeleteMessages.call(this, params).then(function(result) {
        return result;
      }).fail(function(err) {
        linking.linkItems(toRemove, lookup);
        return err;
